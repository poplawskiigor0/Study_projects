...
SpaceForPeople::SpaceForPeople(int floor, SmartBox<Human> passengers, SkyScraper building){
    #ifdef _DEBUG
...
SpaceForPeople::SpaceForPeople(SmartBox<Human> passengers){
    #ifdef _DEBUG
...
/*SpaceForPeople& SpaceForPeople::operator+(SmartBox<Human> crowd) //grupa pasażerów wsiada
{
    m_passengers=crowd;
...
SpaceForPeople& SpaceForPeople::operator=(const SpaceForPeople &e){
    ...
    m_passengers=e.m_passengers;
    m_building=e.m_building;
    //tutaj wywolanie funckji paste
    return *this;
}

bool SpaceForPeople::operator==(const SpaceForPeople &e){
    if(this==&e)
        return true;
    if(!(    m_floor==e.m_floor &&
           m_passengers_count==e.m_passengers_count &&
           //m_passengers==e.m_passengers &&
           m_building==e.m_building)) return false;
}
