...
CleaningPlatform::CleaningPlatform(int floor, double lift_cap, double staff_weight, SmartBox<Human> passengers, SkyScraper building){
    #ifdef _DEBUG
...
CleaningPlatform::CleaningPlatform(SmartBox<Human>){
    #ifdef _DEBUG
...
bool CleaningPlatform::operator==(const CleaningPlatform &e){
    if(this==&e)
        return true;
    if(m_lift_cap==e.m_lift_cap &&
       m_staff_weight==e.m_staff_weight) return true;
    return false;
}
